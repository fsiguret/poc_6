...
const middleware = (req, res, next) => {
    const schema = Joi.object().keys({
        ...
        name: Joi.string().regex(/^[a-zA-Zéèàùüïäëöâêîôû]{3,30}(\s?[a-zA-Zéèàùüïäëöâêîôû]{3,30})*$/i).required(),
        manufacturer: Joi.string().regex(/^[a-zA-Zéèàùüïäëöâêîôû]{3,30}(\s?[a-zA-Zéèàùüïäëöâêîôû]{3,30})*$/i).required(),
